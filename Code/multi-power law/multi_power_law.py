...
if __name__ == "__main__":
    ...
    GRID_CFG = {
        ...
        'beta': np.linspace(0.3, 0.7, 2),   
        'gamma': np.linspace(0.3, 0.7, 2)   
    }
    ...
    pa = multi_power_law(np.arange(len(S1)), S1, LD, bp[0], bp[1], bp[4])
    ta = td['losses']
    ...
    for lt in PREDICT_TYPES:
        ...
        plt.scatter(td['steps'][didx], td['losses'][didx],
                    s=15, ec=scfg[lt]['c'], fc='white',
                    marker=scfg[lt]['m'], lw=1.5,
                    label=f"{scfg[lt]['l']}实际")

    os.makedirs('./figures', exist_ok=True)    
    plt.title(f"基于{FIT_TYPE}的多幂律预测 (R²={r2:.4f})", fontsize=14, pad=15)
